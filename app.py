...
def users_likes(user_id):
    ...
    user = User.query.get_or_404(user_id)
    messages_likes = [message for message in user.likes]
    return render_template('users/likes.html', user=user, messages=messages_likes)
...
def add_like(message_id):
    ...
    if liked_message.user_id == g.user.id:
        ...
        if liked_message not in g.user.likes:
...
def homepage():
    ...
    if g.user:
        ...
        following_users = [followee.id for followee in g.user.following]
